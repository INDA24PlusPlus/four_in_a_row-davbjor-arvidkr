    ...
    pub fn has_won(&self) -> char {
		for i in 0..42 {
			if self.board[i as usize] != '.' {
				...
				let diagl1 = (x-1)+(y+1)*7;
				let diagl2 = (x-2)+(y+2)*7;
				let diagl3 = (x-3)+(y+3)*7;
				...
				let diagr1 = (x+1)+(y+1)*7;
				let diagr2 = (x+2)+(y+2)*7;
				let diagr3 = (x+3)+(y+3)*7;
...
fn main() {
    ...
    let mut board = Board { board: ['.'; 42], turn: true };
    ...
    loop {
        ...
        let send = num - 1;
        if board.place_bricka(send) == false {
